...
def fetch_packages(location_code: str = None) -> list[dict]:
    """
    Fetch base data packages. Optionally filter by locationCode (ISO Alpha-2).
    """
...
def order_esim_open(memo: str, package_code: str, price_usd: float) -> str | None:
    """
    Place an eSIM order via open/esim/order.
    """
    url = "https://api.esimaccess.com/api/v1/open/esim/order"
    headers = {"Authorization": ESIM_API_KEY, "Content-Type": "application/json"}
    amt_units = int(price_usd * 10000)  # API expects price * 10000
    payload = {
        "transactionId": memo,
        "amount":        amt_units,
        "packageInfoList": [{
...
def query_esim_open(order_no: str) -> list[dict]:
    """
    Query allocated eSIM profiles via open/esim/query.
    """
...
async def help_cmd(update: Update, context: CallbackContext):
    await update.message.reply_text(
        ...
        "*/browse* – Browse available data packages\n"
        "*/check* – Check pending payment or order status\n"
        "*/topup <amount>* – Request a top-up (QR + memo)\n"
        "*/topup <user_id> <amount>* – Credit user (admin only)\n"
        "*/admin* – Sales stats (admin only)\n\n"
        "Tip: Use USDT balance for instant orders.",
        parse_mode="Markdown"
    )
...
async def browse(update: Update, context: CallbackContext):
    # list all regions by fetching packages with no filter
    pkgs = fetch_packages()
    regions = sorted({p.get("locationCode","GLOBAL") for p in pkgs})
    kb = [[InlineKeyboardButton(r, callback_data=f"REG_{r}")] for r in regions]
    await update.message.reply_text("🌍 Select region:", reply_markup=InlineKeyboardMarkup(kb))
...
async def region_selector(update: Update, context: CallbackContext):
    ...
    if not pkgs:
        return await update.callback_query.message.reply_text("No packages for this region.")
    kb=[]
    for p in pkgs:
    ...
    await update.callback_query.message.reply_text("📡 Select package:", reply_markup=InlineKeyboardMarkup(kb))
...
async def pkg_handler(update: Update, context: CallbackContext):
    ...
    if bal < price_usd:
        ...
        conn.commit()
        txt = (f"🔋 Top-up to order\nSend *{price_usd:.2f} USDT* to:\n"
               f"`{WALLET_ADDRESS}`\nMemo: `{memo}`")
        return await query.message.reply_photo(photo=send_qr_code(txt), caption=txt, parse_mode="Markdown")
    ...
    if not order_no:
        return await query.message.reply_text("❌ Order failed.")

    c.execute("UPDATE orders SET memo=?, order_no=?, paid=1 WHERE user_id=? AND plan_id=? AND paid=0",
    ...
    if not profiles:
        return await query.message.reply_text(
            f"✔️ Order {order_no} placed. Profiles still allocating; use /check."
        )
...
async def check(update: Update, context: CallbackContext):
    ...
    c.execute("SELECT order_no FROM orders WHERE user_id=? AND paid=1 AND order_no IS NOT NULL ORDER BY id DESC LIMIT 1", (user_id,))
    row = c.fetchone()
    if not row:
        return await update.message.reply_text("No recent orders.")
    order_no = row[0]
    profiles = query_esim_open(order_no)
    if not profiles:
        return await update.message.reply_text("⏳ Still allocating; try again.")
    for p in profiles:
        qr = p.get("qrCodeUrl") or p.get("ac")
...
async def topup(update: Update, context: CallbackContext):
    ...
    if len(args)==1:
        amt = float(args[0]); memo=generate_memo()
        ...
        conn.commit()
        txt = (f"🔋 Top-Up Request\nSend *{amt:.2f} USDT* to:\n"
               f"`{WALLET_ADDRESS}`\nMemo: `{memo}`")
        return await update.message.reply_photo(photo=send_qr_code(txt), caption=txt, parse_mode="Markdown")
...
app.add_handler(CallbackQueryHandler(country_selector, pattern="^CNTRY_"))
